...
Architecture:
  - patch_model(): entry point. Patches two things:
      1. prepare_inputs_labels_for_multimodal_new -> records the (start, end)
         span of visual tokens in module-level state. Doesn't change return value.
      2. Qwen2Model.forward -> our replacement that runs layer 0, prunes, then
         runs layers 1-27 with sliced mask and renumbered position_ids.

  - The replacement Qwen2Model.forward is a near-verbatim copy of the
    transformers 4.37.2 source. We make three changes:
      ...
      c) position_ids are recomputed unconditionally in the pruned-cache
         frame from past_key_values_length. HF generate passes unpruned-
         frame position_ids at decode that would index past our pruned
         rotary tables; we ignore HF's value and use our own.
...
Known approximation (revisit if accuracy is unexpectedly degraded):
  - Layer 0's K-cache retains the RoPE rotations applied at original
    sequence positions. After pruning, Q-at-pruned-position attends to
    K-cache entries rotated at original positions. This is the same
    approximation made by FastV and most published pruning methods.
...
# Number of visual tokens HuatuoGPT-Vision produces per image: 576
# (CLIP ViT-L/14 @ 336^2, 24x24 patches).
IMAGE_TOKEN_INDEX = -200
N_VISUAL_TOKENS = 576


# Module-level state. The prepare_inputs_labels_for_multimodal_new patch
# writes here; the Qwen2Model.forward patch reads from here. We never run
# more than one model in one process so this is safe.
_STATE = {
    "pruner": None,
...
# Once-per-run sentinel. Prints a single line at the start of each run so
# logs visibly confirm pruning fired, then stays silent. Per-sample
# verification lives in the LatencyTracker JSON (n_visual_pre / post).
...
# --------------------------------------------------------------------------- #
# Patch 1: prepare_inputs_labels_for_multimodal_new
# --------------------------------------------------------------------------- #
...
def _patch_prepare_inputs(model):
    """Patch HuatuoGPT-Vision's custom multimodal-prep method to record
    visual/question spans. HuatuoGPT-Vision routes forward() and generate()
    through prepare_inputs_labels_for_multimodal_new(), NOT the stock LLaVA
    prepare_inputs_labels_for_multimodal() — patching the latter attaches
    to a dead branch."""
...
def _build_pruned_forward():
    ...
    def pruned_forward(
        ...
        # OVERRIDE: always compute position_ids in the pruned-cache frame,
        # ignoring whatever HF generate passed in. HF computes position_ids
        # from the 2D attention_mask via cumsum-1, so at decode it contains
        # values like [631, 632] (original unpruned frame). Our KV cache is
        # in the pruned frame and the rotary cos/sin tables are sized to
        # past_key_values_length + seq_length. Indexing the rotary tables
        # with 631/632 when they're only 346 long → CUDA index out of bounds.
        # The arange below stays inside the cache frame: prefill gets
        # [0..seq_length-1], decode gets [past_kv_len..past_kv_len+seq_len-1].
        ...
        # Compensate for pruning during decode steps. HuggingFace's generate()
        # loop tracks the 2D attention_mask in the ORIGINAL (unpruned) prompt
        # frame, extending it by 1 per decode token. But our KV cache holds
        # only pruned_prompt_len entries, so past_key_values_length is in the
        # pruned frame. Slice the 2D mask down to the rightmost
        # (past_key_values_length + seq_length) entries so the 4D mask
        # construction below sees consistent lengths.
        #
        # During prefill, attention_mask is None (HuatuoGPT-Vision passes
        # attention_mask=None to super().generate), so this block is a no-op
        # and prefill is unaffected.
        if (
            attention_mask is not None
            and attention_mask.dim() == 2
            and attention_mask.shape[-1] > past_key_values_length + seq_length
        ):
            attention_mask = attention_mask[..., -(past_key_values_length + seq_length):]
...
def _apply_pruning_step(hidden_states, attention_mask, position_ids, seq_length, attn_impl, pruner, latency_tracker, past_key_values=None):
    ...
    if not _FIRST_PRUNE_LOGGED:
        print(f"[PATCHER] first prune confirmed: seq {seq_length} -> {new_seq_len}, "
              f"visual {n_visual} -> {int(keep_visual[0].sum().item())} "
...
def patch_model(model):
    ...
    Args:
        model: the HuggingFace model that has .prepare_inputs_labels_for_multimodal_new
               and .model (the Qwen2Model). For HuatuoGPT-Vision-7B this is the
               LlavaQwen2ForCausalLM-equivalent module.
    """
